...
class PacketCreator(object):
    ...
    def _checkReceivedPacketSize(self, packet, packetType):
        if packetType == PacketType.FwDownloadVerifiedPacket:
            if len(packet) != 8:
                raise ValueError("FwDownloadVerifiedPacket size is incorrect" + str(len(packet)))
        if packetType == PacketType.FwVersionPacket:
            if len(packet) != 23:
    ...
    def _getFramesList(self, binFile):
        ...
        framesList = [fileData[i:i + self.frameLength] for i in range(0, fileSize, self.frameLength)]
        return framesList
