...
mock_command_runner = modules["pitop.utils.command_runner"] = Mock()
mock_logger = modules["pitop.utils.logger"] = Mock()
mock_i2c = modules["pitop.utils.i2c_device"] = Mock()
...
class FirmwareUpdaterFunctionsTestCase(TestCase):
    ...
    def test_create_fw_device_succeeds_if_connected(self):
        ...
        FirmwareDevice.get_part_name.return_value = \
    ...
    def test_create_fw_updater_calls_create_fw_device(self):
        ...
        with patch.object(pt_firmware_updater, 'create_firmware_device', wraps=pt_firmware_updater.create_firmware_device) as wrapped_call:
    ...
    def test_create_fw_updater_succeeds_if_connected(self):
        device_to_mock = FirmwareDeviceID.pt4_hub
        FirmwareDevice.get_part_name = Mock(side_effect=[
            FirmwareDevice.device_info.get(device_to_mock).get("part_name")
        ])
        pt_firmware_updater.FirmwareUpdater.set_current_device_info = Mock()
...
class FirmwareUpdaterFlowsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        pt_firmware_updater.FirmwareUpdater.set_current_device_info = Mock()
        pt_firmware_updater.FirmwareUpdater.install_updates = Mock(return_value=(True, False))
        pt_firmware_updater.FirmwareUpdater.device_info = FirmwareFileObject.from_file(self.path_to_fw_device)
        pt_firmware_updater.FirmwareUpdater.fw_downloaded_successfully = Mock(return_value=False)
        self.send_notification_mock = core.notification_manager.send_notification = Mock(return_value="1")
    ...
    def test_doesnt_update_if_user_rejects_notification(self):
        ...
        with patch.object(pt_firmware_updater, 'apply_update', wraps=pt_firmware_updater.apply_update) as wrapped_apply_update:
            pt_firmware_updater.main(parsed_args)
    ...
    def test_notifies_user_if_update_failed(self):
        self.send_notification_mock = core.notification_manager.send_notification = Mock(return_value="0\nOK")
