...
class PacketManager(object):
    ...
    def _create_starting_packet(self):
        ...
        fw_size = PacketManager.int_to_hex_string(
            os.path.getsize(self.bin_file), 4)
        frame_size = PacketManager.int_to_hex_string(self.frame_length, 2)
        total_frames = PacketManager.int_to_hex_string(len(self._get_frames_list()), 2)
        last_frame = PacketManager.int_to_hex_string(
            len(self._get_frames_list()[-1]), 2)
        fw_checksum = self._get_firmware_checksum()
        reserved = PacketManager.int_to_hex_string(0, 2)
